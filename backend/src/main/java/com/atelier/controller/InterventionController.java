...
public class InterventionController {
    ...
    public ResponseEntity<InterventionDTO> diagnostiquer(
            @PathVariable Long id,
            @RequestBody DiagnostiquerRequest request,
            @AuthenticationPrincipal UtilisateurPrincipal principal) {
        Utilisateur auteur = principal.getUtilisateur();
        InterventionDTO miseAJour = interventionService.diagnostiquer(id, request, auteur);
        return ResponseEntity.ok(miseAJour);
    }

    @GetMapping("/{id}/historique")
    public ResponseEntity<List<HistoriqueInterventionDTO>> getHistorique(@PathVariable Long id) {
        return ResponseEntity.ok(interventionService.getHistorique(id));
    }
