...
import com.atelier.dto.request.DiagnostiquerRequest;
...
import com.atelier.mapper.HistoriqueInterventionMapper;
...
public class InterventionService {
    ...
    private final HistoriqueInterventionMapper historiqueMapper;
    ...
    public InterventionDTO changerStatut(Long interventionId, ChangerStatutRequest request, Utilisateur auteur) {
        ...
        if (statutCible == StatutIntervention.DEVIS_A_VALIDER
                && (intervention.getDiagnostic() == null || intervention.getDiagnostic().isBlank())) {
            throw new ReglesMetierException("Le diagnostic doit être renseigné avant le statut Devis à valider.");
        }
        ...
        if (request.getDateRestitutionPrevue() != null) {
            intervention.setDateRestitutionPrevue(request.getDateRestitutionPrevue());
        }
    ...
    @Transactional
    public InterventionDTO diagnostiquer(Long interventionId, DiagnostiquerRequest request, Utilisateur auteur) {
        Intervention intervention = interventionRepository.findById(interventionId)
                .orElseThrow(() -> new ReglesMetierException("Intervention introuvable : id=" + interventionId));

        if (intervention.getStatut() == StatutIntervention.RESTITUEE
                || intervention.getStatut() == StatutIntervention.ANNULEE) {
            throw new ReglesMetierException("Impossible de modifier le diagnostic d'une intervention clôturée.");
        }

        intervention.setDiagnostic(request.getDiagnostic());
        return interventionMapper.toDto(interventionRepository.save(intervention));
    }

    @Transactional(readOnly = true)
    public List<HistoriqueInterventionDTO> getHistorique(Long interventionId) {
        if (!interventionRepository.existsById(interventionId)) {
            throw new ReglesMetierException("Intervention introuvable : id=" + interventionId);
        }
        return historiqueRepository.findByInterventionIdOrderByDateDesc(interventionId).stream()
